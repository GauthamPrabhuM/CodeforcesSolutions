...
void pre()
{
  ...
  for(int i=4;i<=20;i++)
  {
    if(fac.back()*1ll*i <= 1e12)
     fac.push_back(fac.back()*1ll*i);
  }
}
    ...
    void solve()
    {
        ...
        /* Value ti shows the time the Rabbits need to lunch in the i-th restaurant. If time ti exceeds the time k 
        that the coach has given for the lunch break, then the Rabbits' joy from lunching in this restaurant will equal 
        fi-ti+k
        . Otherwise, the Rabbits get exactly fi units of joy.*/
        ll n;
        ll k;
        cin>>n>>k;
        ll ans=-10000006;
