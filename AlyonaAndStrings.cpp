...
int N, M, K, dp[1000][1000][11][2];
string S, T;
int sol(int i, int j, int rem, int flag)
{
    if (i == N || j == M)
        return 0;
    int& ret = dp[i][j][rem][flag];
    if (~ret)
        return ret;
    ret = sol(i + 1, j, rem, 0);
    ret = max(ret, sol(i, j + 1, rem, 0));
    if (S[i] == T[j])
    {
        if (rem)
            ret = max(ret, 1 + sol(i + 1, j + 1, rem - 1, 1));
        if (flag) 
            ret = max(ret, 1 + sol(i + 1, j + 1, rem, 1));
    }
    return ret;
}
